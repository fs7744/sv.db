...
namespace UT.GeneratorTestCases
{
    public class NullableFieldTestCase
    {
        ...
        public void Check(string generatedCode)
        {
            ...
            Assert.Contains("d.Int = reader.IsDBNull(j) ? default(int? ) : reader.GetInt32(j);", generatedCode);
            Assert.Contains("d.Int = reader.IsDBNull(j) ? default(int? ) : DBUtils.As<int>(reader.GetValue(j));", generatedCode);
